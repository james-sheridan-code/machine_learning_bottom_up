...
Model:          y = XW + b
Cost Function:  Mean Squared Error (MSE) + Ridge term (L2)
...
def batch_gradient_descent(W, b, learn_rate, iterations, y_train, X_train, reg_lambda):
    ...
    with the ridge regularisation term (L2) computed using all training 
    ...
    for i in range(iterations):
        ...
        # MSE Cost Function with Ridge Term
        cost = 0.5 * np.mean((y_pred - y_train)**2) + (reg_lambda / (2 * m)) * np.sum(W**2)

        # Formula: dw = (1/m) * X.T @ (y_pred - y_train)
        # Shapes: (4,1) = scalar * (3,4).T @ ((3,1) - (3,1)) + scalar * (4,1)
        dw = (1/m) * (X_train.T @ (y_pred - y_train)) + (reg_lambda/m) * W
